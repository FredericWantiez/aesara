...
BTRDParams = namedtuple(
    "BTRDParams", ["a", "b", "c", "spq", "vr", "ur", "m", "logp", "log1p", "alpha"]
)
...
def btrd(key, n, p):
    ...
    def body_fun(val):
        ...
        u2, v2, nkey = lax.cond(
            v >= params.vr, left_branch, right_branch, (0.5, v, vkey)
        )
        k2 = jnp.floor(G(u2))
        v, k = lax.cond(
            v < 2 * params.ur * params.vr, lambda _: (v, k1), lambda _: (v2, k2), (0, 0)
        )
        return k, nkey, u2, v
    ...
    k, _, _, _ = lax.while_loop(cond_fun, body_fun, (-1, key, 0, 0))
    return k.astype(int)
